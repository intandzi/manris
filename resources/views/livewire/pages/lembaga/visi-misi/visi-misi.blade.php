<div>
    ...
    @include('livewire.pages.lembaga.visi-misi.visi-misi-modal')
        ...
        <nav aria-label="breadcrumb" class="mb-2">
            <ol class="breadcrumb mb-0 p-2">
                <li class="breadcrumb-item"><a href="#"><i class="mdi mdi-apps"></i>
                        App</a></li>
                <li class="breadcrumb-item active"><a href="{{ route('visimisi.index', ['role' => request()->query('role')]) }}"
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-inline-flex gap-2">
                                                    <button type="button"
                                                        wire:click.prevent="create({{ $unit->unit_id }})"
                                                        wire:loading.attr="disabled"
                                                        wire:target="create({{ $unit->unit_id }})"
                                                        class="btn btn-warning btn-sm d-flex text-center align-items-center">
                                                        <i class="ri-pencil-fill" wire:loading.remove
                                                            wire:target='create({{ $unit->unit_id }})'>
                                                        </i>
                                                        <span wire:loading wire:target="create({{ $unit->unit_id }})">
                                                            <span class="spinner-border spinner-border-sm"
                                                                role="status" aria-hidden="true"></span>
                                                        </span>
                                                    </button>
                                                    @if ($unit->visimisi->isNotEmpty())
                                                        <button type="button"
                                                            wire:click.prevent="show({{ $unit->unit_id }})"
                                                            wire:loading.attr="disabled"
                                                            wire:target="show({{ $unit->unit_id }})"
